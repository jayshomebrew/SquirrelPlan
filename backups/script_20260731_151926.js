    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function onBlurNumber(event, rules) {
            ...
            if (input.value) {
                ...
                const newVal = formatNumberForDisplay(input.value, rules);
                if (oldVal !== newVal) {
                    input.value = newVal;
        ...
        function syncAllocationPeriodsAssets() {
            document.querySelectorAll('.allocation-period').forEach(periodRow => {
                const currentAllocations = {};
                periodRow.querySelectorAll('.allocation-asset').forEach(input => {
                    if (input.dataset.assetName) {
                        currentAllocations[input.dataset.assetName] = input.value;
                    }
                });
                updateAllocationAssets(periodRow, currentAllocations);
            });
        }
        ...
        function addCategory(container, type, category = {}) {
            ...
            switch (type) {
                ...
                case 'pension': fields = getPensionFields(category); break;
            }
            ...
            if (type === 'income') {
                ...
                const endYearInput = row.querySelector('.category-end-year');
                if (endYearInput) manageSlider(endYearInput);
                // Update value slider limits when frequency (monthly/annual) changes
                const frequencySelect = row.querySelector('.category-frequency');
                if (frequencySelect && valueInput) {
                    frequencySelect.addEventListener('change', () => {
                        manageSlider(valueInput);
                    });
                }
            }
        ...
        function getIncomeFields(category) {
            ...
            const incomeName = category.incomeName || '';
            return `
                <div class="col-6 col-md-4 col-lg-3 col-xxl-2"><label for="${idPrefix}-name" class="form-label" data-bs-toggle="tooltip" data-i18n-title-key="incomeNameTooltip">${getTranslation('nameLabel')} <i class="bi bi-info-circle"></i></label><input type="text" id="${idPrefix}-name" class="form-control category-name" value="${incomeName}"></div>
        ...
        function getPensionFields(category) {
            const idPrefix = getUniqueId('pension');
            const isPension = category.pensionType === 'pension';
            return `
                <div class="col-6 col-md-4 col-lg-3 col-xxl-2"><label for="${idPrefix}-name" class="form-label" data-bs-toggle="tooltip" data-i18n-title-key="pensionNameTooltip">${getTranslation('nameLabel')} <i class="bi bi-info-circle"></i></label><input type="text" id="${idPrefix}-name" class="form-control category-name" value="${category.name || ''}"></div>
                <div class="col-6 col-md-4 col-lg-3 col-xxl-2"><label for="${idPrefix}-value" class="form-label" data-bs-toggle="tooltip" data-i18n-title-key="pensionValueTooltip">${getTranslation('valueLabel')} <i class="bi bi-info-circle"></i></label><input type="text" id="${idPrefix}-value" class="form-control category-value" value="${category.value || ''}"></div>
                <div class="col-6 col-md-4 col-lg-3 col-xxl-2"><label for="${idPrefix}-frequency" class="form-label" data-bs-toggle="tooltip" data-i18n-title-key="pensionFrequencyTooltip">${getTranslation('frequencyLabel')} <i class="bi bi-info-circle"></i></label><select id="${idPrefix}-frequency" class="form-select category-frequency"><option value="monthly" ${category.frequency === 'monthly' ? 'selected' : ''}>${getTranslation('monthlyOption')}</option><option value="annual" ${category.frequency === 'annual' ? 'selected' : ''}>${getTranslation('yearlyOption')}</option></select></div>
                <div class="col-6 col-md-4 col-lg-3 col-xxl-1"><label for="${idPrefix}-indexed" class="form-label" data-bs-toggle="tooltip" data-i18n-title-key="pensionIndexedTooltip">${getTranslation('indexedLabel')} <i class="bi bi-info-circle"></i></label><input id="${idPrefix}-indexed" class="form-check-input d-block category-indexed" type="checkbox" ${category.indexed ? 'checked' : ''}></div>
                <div class="col-6 col-md-4 col-lg-3 col-xxl-1"><label for="${idPrefix}-pension-type" class="form-label">${getTranslation('pensionTypeLabel') || 'Type'} <i class="bi bi-info-circle"></i></label>
                    <div class="form-check form-switch">
                        <input class="form-check-input category-pension-type" type="checkbox" id="${idPrefix}-pension-type" ${isPension ? 'checked' : ''}>
                        <label class="form-check-label" for="${idPrefix}-pension-type">${getTranslation('pensionOption') || 'Pension'}</label>
                    </div>
                </div>
                <div class="col-6 col-md-4 col-lg-3 col-xxl-1"><label for="${idPrefix}-start-year" class="form-label" data-bs-toggle="tooltip" data-i18n-title-key="pensionStartYearTooltip">${getTranslation('startLabel')} <i class="bi bi-info-circle"></i></label><input type="text" id="${idPrefix}-start-year" class="form-control category-start-year" value="${category.startYear || ''}"><small class="text-muted">${getTranslation('pensionAgeLabel') || 'Pension Age'}</small></div>
                <div class="col-6 col-md-4 col-lg-3 col-xxl-1 d-flex align-items-end"><i class="bi bi-grip-vertical drag-handle me-2" style="cursor: move; font-size: 1.2rem;"></i><button class="btn btn-danger btn-sm delete-category-btn" aria-label="${getTranslation('deletePensionCategoryLabel')}"><i class="bi bi-trash"></i></button></div>
            `;
        }
        ...
        function updateAllocationAssets(periodRow, allocation) {
            ...
            assetNames.forEach(name => {
                ...
                const val = allocation[name] !== undefined ? allocation[name] : 0;
                div.innerHTML = `
                    <span class="input-group-text">${name} %</span>
                    <input type="text" class="form-control allocation-asset" data-asset-name="${name}" value="${val}">
                `;
            ...
            const getCategoryData = (container, type) => {
                ...
                container.querySelectorAll('.category-row').forEach((row, idx) => {
                    ...
                    if (type === 'income' || type === 'expense') {
                        ...
                        const personField = row.querySelector('.category-person');
                        item.person = personField ? personField.value : '';
                        const incomeNameField = row.querySelector('.category-name');
                        item.incomeName = incomeNameField ? incomeNameField.value : '';
                    ...
                    const startYearInput = row.querySelector('.category-start-year');
                    item.startYear = startYearInput ? parseInt(parseFormattedNumber(startYearInput.value)) || 0 : 0;
                    const endYearInput = row.querySelector('.category-end-year');
                    item.endYear = endYearInput ? parseInt(parseFormattedNumber(endYearInput.value)) || 0 : 0;
                    items.push(item);
                });
                return items;
            };
        ...
        function runAndRender(isManualRun = false) {
            ...
            const yearSlider = document.getElementById('summary-year-slider');
            const yearLabel = document.getElementById('summary-year-label');
            const ageLabel = document.getElementById('summary-age-label');

            if (yearSlider && results && results.length > 0) {
                ...
                if (isNaN(selectedYear) || selectedYear < startYear || selectedYear > endYear) {
                    selectedYear = startYear;
                    yearSlider.value = selectedYear;
                }

                const currentResult = results.find(r => r.year === selectedYear) || results[0];
                if (yearLabel) yearLabel.textContent = currentResult.year;
                if (ageLabel) ageLabel.textContent = currentResult.age;

                if (typeof window.renderSankeyChart === 'function') {
                    window.renderSankeyChart(inputs, selectedYear);
                }
            } else if (typeof window.renderSankeyChart === 'function') {
                window.renderSankeyChart(inputs);
            }
        ...
        function setUserValues(data) {
            ...
            data.pensions.forEach(p => {
                const modP = { ...p, value: String(p.value) };
                addCategory(pensionCategoriesContainer, 'pension', modP);
            });
        ...
        function renderTable(results) {
            ...
            let headerHtml = `<th>${getTranslation('yearHeader')}</th><th>${getTranslation('ageHeader')}</th><th>${getTranslation('netIncomeHeader')}</th><th>${getTranslation('expensesHeader')}</th><th>${getTranslation('savingsCapacityHeader')}</th><th>Saved to Assets</th><th>${getTranslation('savingsRateHeader')}</th><th>${getTranslation('netWorthHeader')}</th>`;
        ...
        function initDefaultData(stage = 'early-career') {
            ...
            data.pensions.forEach(p => addCategory(pensionCategoriesContainer, 'pension', p));
        ...
        function initializeSortable() {
            const options = {
                ...
                handle: '.drag-handle',
                onEnd: () => autoRunSimulationIfEnabled()
            };
            ...
            new Sortable(pensionCategoriesContainer, options);
        }
        ...
        document.getElementById('add-income-category-btn').addEventListener('click', () => {
            const personName = document.getElementById('person-name').value || 'Person1';
            addCategory(incomeCategoriesContainer, 'income', { name: personName });
        });
        document.getElementById('add-expense-category-btn').addEventListener('click', () => addCategory(expenseCategoriesContainer, 'expense'));
        document.getElementById('add-pension-category-btn').addEventListener('click', () => {
            const personName = document.getElementById('person-name').value;
            const defaultName = personName || 'Person1';
            addCategory(pensionCategoriesContainer, 'pension', { name: defaultName });
        });
        ...
        document.getElementById('financial-data-content').addEventListener('input', autoRunSimulationIfEnabled);
        document.getElementById('financial-data-content').addEventListener('change', autoRunSimulationIfEnabled);

        const summarySlider = document.getElementById('summary-year-slider');
        if (summarySlider) {
            summarySlider.addEventListener('input', function () {
                const inputs = getUserInputs();
                const selectedYear = parseInt(this.value);
                const startYear = new Date().getFullYear();
                const currentAge = (inputs.currentAge || 0) + (selectedYear - startYear);

                const yearLabel = document.getElementById('summary-year-label');
                const ageLabel = document.getElementById('summary-age-label');
                if (yearLabel) yearLabel.textContent = selectedYear;
                if (ageLabel) ageLabel.textContent = currentAge;
        ...
        function manageSlider(input) {
            ...
            switch (input.id) {
                ...
                case 'pension-age': min = 62; max = 70; step = 1; break;
                ...
                case 'early-retirement-age':
                    ...
                    max = parseInt(document.getElementById('pension-age').value) || 70;
                    break;
                default:
                    if (input.id.includes('-value')) {
                        // Income category: adjust slider max based on monthly vs annual frequency
                        const row = input.closest('.category-row');
                        if (row && row.dataset.type === 'income') {
                            const freqSelect = row.querySelector('.category-frequency');
